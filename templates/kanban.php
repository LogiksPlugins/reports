    ...
    <div class='kanbanCardTemplate hidden'>
    	<?php
    		if(isset($reportConfig['kanban']['template'])) {
    			...
    			?>
    			<article class="dataItem kanban-entry grab rColor {{cardColor <?=$colMap['color']?>}}" id="item{{id}}" data-hash='{{hashid}}' data-refid='{{id}}'>
...
function generateKanbanUI(rpt) {
	...
	lx=_service("reports","enumerateColumn","json")+"&gridid="+rpt.gridID+"&colKey="+pivot;
